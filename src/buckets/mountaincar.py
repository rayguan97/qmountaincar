...
ap.add_argument("-me", "--buckets",type = int, default = 40)
args = vars(ap.parse_args())
x = args["buckets"] # exploration rate
# hyperparameters
buckets=(x, x,) #this is a continuous case, we need to give them a range
n_episodes=10000
goal_reward=-195
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        # check if our policy is good
            if mean_reward >= goal_reward or episode >= 2000:
                print('Ran {} episodes. Solved after {} trials with buckets {}'.format(episode, episode - 100, x))
                print(mean_reward)
                Datapath = "data/"
                data.append(episode)
                # visualize_policy()
    ...
    open('%sdata-run-%s.csv' % (Datapath, x), 'wb') 
    np.savetxt('%sdata-run-%s.csv' % (Datapath, x), data)
            #elif episode % 100 == 0:
