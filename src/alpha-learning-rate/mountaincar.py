...
ap.add_argument("-me", "--min_alpha",type = float, default = 0.3)
args = vars(ap.parse_args())
min_alpha = args["min_alpha"] # exploration rate
# hyperparameters
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        # check if our policy is good
            if mean_reward >= goal_reward and episode >= 100:
                print('Ran {} episodes. Solved after {} trials with min_alpha {}'.format(episode, episode - 100, min_alpha))
                print(mean_reward)
                Datapath = "data/"
                data.append(episode)
                # visualize_policy()
    ...
    open('%sdata-run-%s.csv' % (Datapath, min_alpha), 'wb') 
    np.savetxt('%sdata-run-%s.csv' % (Datapath, min_alpha), data)
            #elif episode % 100 == 0:
