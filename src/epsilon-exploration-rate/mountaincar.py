...
ap.add_argument("-me", "--min_epsilon",type = float, default = 0.1)
args = vars(ap.parse_args())
min_epsilon = args["min_epsilon"] # exploration rate
# hyperparameters
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        # check if our policy is good
            if mean_reward >= goal_reward and episode >= 100:
                print('Ran {} episodes. Solved after {} trials with min_epsilon {}'.format(episode, episode - 100, min_epsilon))
                print(mean_reward)
                Datapath = "data/"
                data.append(episode)
                visualize_policy()
    ...
    open('%sdata-run-%s.csv' % (Datapath, min_epsilon), 'wb') 
    np.savetxt('%sdata-run-%s.csv' % (Datapath, min_epsilon), data)
            #elif episode % 100 == 0:
