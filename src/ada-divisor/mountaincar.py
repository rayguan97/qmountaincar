...
ap.add_argument("-me", "--ada_divisor",type = int, default = 25)
args = vars(ap.parse_args())
ada_divisor = args["ada_divisor"] # exploration rate
# hyperparameters
buckets=(40, 40,) #this is a continuous case, we need to give them a range
n_episodes=10000
goal_reward=-195
min_alpha=0.6  # learning rate
min_epsilon=0.0
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        # check if our policy is good
            if mean_reward >= goal_reward and episode >= 100:
                print('Ran {} episodes. Solved after {} trials with ada-divisor {}'.format(episode, episode - 100, ada_divisor))
                print(mean_reward)
                Datapath = "data/"
                data.append(episode)
                # visualize_policy()
    ...
    open('%sdata-run-%s.csv' % (Datapath, ada_divisor), 'wb') 
    np.savetxt('%sdata-run-%s.csv' % (Datapath, ada_divisor), data)
            #elif episode % 100 == 0:
